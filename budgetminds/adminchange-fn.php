...
if(empty($_SESSION['id']))
{
 ...
 $req = $bdd->prepare('SELECT * FROM admin WHERE id=:session');
 ...
 if(!empty($_POST))
 {
	...
	if(!empty($_POST['pass']) && !empty($_POST['confirm_password']) && !empty($_POST['login']) && !empty($_POST['email'])) {
  ...
  if(empty($_POST['pass']) || empty($_POST['confirm_password']) || empty($_POST['login']) || empty($_POST['email'])) {
	...
	if($valid)
	{
		...
		$req = $bdd->prepare('UPDATE admin SET pass=:pass, login=:login, email=:email WHERE id=:session');
		$req->execute(array(
			'pass'=>md5($pass),
			'login'=>$login,
			'email'=>$email,
		...
		$req->closeCursor();
		$ok = 'Modification réussie ! <a href="adminlogin.php" title="Accueil">Veuillez vous reconnecter</a>';
			...
			<div id="main-depenses">
				...
				<div class="clipboard"><a href="adminindex.php" title="Les membres"><img src="img/members.svg" alt="Les membres" /></a></div>
					<div id="mesdepenses">
						...
						<form id="signupForm" action="adminchange-fn.php" method="post">
